...
fn probe_167_energy_motifs() {
    ...
    let sc = aspace_eng.sub_centroids.as_ref().unwrap();
    let n_sc = sc.shape().0;
    println!(
        "PROBE nitems={} n_clusters={} n_subcentroids={} reduced_dim={:?}",
        aspace_eng.nitems, aspace_eng.n_clusters, n_sc, aspace_eng.reduced_dim
    );

    // rebuild exactly like try_spot_motives_energy does
    let mut params = gl_eng.graph_params.clone();
    params.k = params.k.min(n_sc - 1);
    params.topk = params.topk.min(4).min(n_sc - 1);
    let g = crate::laplacian::build_laplacian_matrix(sc.clone(), &params, Some(n_sc), true);
    let degs: Vec<usize> = (0..n_sc).map(|i| g.neighbors_of(i).len()).collect();
    println!("PROBE sc-graph degrees={:?} nnz={}", degs, g.matrix.nnz());

    for (tri, clust) in [(1usize, 0.1f64), (1, 0.2), (2, 0.35)] {
        let cfg = MotiveConfig {
            top_l: 16,
            min_triangles: tri,
            min_clust: clust,
        ...
        let m = gl_eng.try_spot_motives_energy(&aspace_eng, &cfg).unwrap();
        println!(
            "PROBE cfg tri={tri} clust={clust}: motifs={} sizes={:?}",
            m.len(),
            m.iter().map(|s| s.len()).collect::<Vec<_>>()
        );
